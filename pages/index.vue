...
export default {
  ...
  head(){
    return {
      title: 'Movie App - Latest Streaming Movie info here',
      meta: [
        {
          hid: 'description',
          name: 'description',
  ...
  methods: {
    async getMovies(){
      const data = axios.get("https://api.themoviedb.org/3/movie/now_playing?api_key=6031db768c9f0a351a4250395d39881e&language=en-US&page=1");
    ...
    async searchMovies(){
      const data = axios.get(`https://api.themoviedb.org/3/search/movie?api_key=6031db768c9f0a351a4250395d39881e&language=en-US&page=1&query=${this.searchInput}`);
      const result = await data;
      result.data.results.forEach((movie)=>{
